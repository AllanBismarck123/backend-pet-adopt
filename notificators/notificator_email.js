...
async function sendEmails(ngoName, subject, text, html) {
  try {
...
async function notificatorSendRequestUser() {
  // var animal = await readAnimalById(request.animalId);
  ...
  await sendEmails('ngo.ngoName', subject, text, html);

}

async function notificatorSendRequestNgo() {
  // var animal = await readAnimalById(request.animalId);
  // var ngo = await readUserById(ngoId);
...
async function notificatorAcceptAdoptUser() {
  // var animal = await readAnimalById(request.animalId);
  // var ngo = await readUserById(ngoId);
  ...
  var text = `
    Prezado(a) [request.tutor.tutorName],

    Temos o prazer de informar que sua solicitação de adoção foi aceita pela ONG [ngo.ngoName]! Você está prestes a dar um novo lar amoroso a [animal.animalName].
    ...
    Enquanto aguarda a adoção, você pode começar a preparar seu lar para receber o [animal.animalName]. Certifique-se de seguir as orientações de cuidado e atenção necessárias para garantir uma transição suave.
  ...
  <body>
      ...
      <p>Prezado(a) [request.tutor.tutorName],</p>

      <p>Temos o prazer de informar que sua solicitação de adoção foi aceita pela ONG [ngo.ngoName]! Você está prestes a dar um novo lar amoroso a [animal.animalName].</p>
      ...
      <p>Enquanto aguarda a adoção, você pode começar a preparar seu lar para receber o [animal.animalName]. Certifique-se de seguir as orientações de cuidado e atenção necessárias para garantir uma transição suave.</p>
      ...
      <p>Atenciosamente,</p>
      <p>[Notificador de E-mails]</p>
      <p>[Assistente de Adoção]</p>
      <p>[ngo.ngoName]</p>
  </body>
...
async function notificatorAcceptAdoptNgo() {
  // var animal = await readAnimalById(request.animalId);
  // var ngo = await readUserById(ngoId);
  ...
  var text = `
    ...
    Gostaríamos de informar que a solicitação de adoção para o animal [{animal.animalName}] foi aceita. O usuário [{request.tutor.tutorName}] demonstrou interesse em adotar o animal e nós aprovamos a solicitação.
  ...
  <body>
      ...
      <p>
          Gostaríamos de informar que a solicitação de adoção para o animal <strong>[animal.animalName]</strong> foi aceita. O usuário <strong>[request.tutor.tutorName]</strong> demonstrou interesse em adotar o animal e nós aprovamos a solicitação.
      </p>

      <p>
          Agradecemos por seu incrível trabalho em cuidar dos animais e fornecer a eles um ambiente seguro e amoroso. Por favor, entre em contato com o usuário <strong>[request.tutor.tutorName]</strong> o mais rápido possível para discutir os detalhes da adoção e agendar uma visita, se necessário.
      </p>
...
async function notificatorRejectAdopt() {
  // var animal = await readAnimalById(request.animalId);
  // var ngo = await readUserById(ngoId);
